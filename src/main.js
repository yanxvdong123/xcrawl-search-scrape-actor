...
// Parse formats
const rawFormats = Array.isArray(formats) ? formats : String(formats).split(',').map(f => f.trim()).filter(Boolean);
let formatsList = rawFormats.length > 0 ? rawFormats : ['markdown', 'summary'];
...
/**
 * Detect if scraped content is likely a captcha/block page.
 * Uses heuristic: very short content + multiple block keywords.
 */
function isLikelyBlockedPage(content) {
  const trimmed = (content || '').trim();
  if (trimmed.length < 150) return true;

  const blockKeywords = [
    'captcha-delivery.com', 'please verify you are human',
    'attention required', 'your request has been blocked',
    'access denied', 'this site may be hacked',
    'sorry, you have been blocked', 'enable javascript',
    'cf-browser-verification', 'just a moment',
    'checking your browser', 'please turn javascript on',
    'browser check', 'cloudflare',
  ];
  const lower = trimmed.toLowerCase();
  const blockCount = blockKeywords.filter(kw => lower.includes(kw)).length;
  return blockCount >= 3;
}
...
function buildScrapeOptions(u, enableRender = false) {
  const body = {
    ...
    request: {
      onlyMainContent: true,   // FIXED: was only_main_content
      blockAds: true,          // FIXED: was block_ads
      device: 'desktop',
    },
    proxy: { location },
  };
  ...
  if (enableRender || render) {
    body.jsRender = {          // FIXED: was js_render
      enabled: true,
  ...
  if (screenshot && (enableRender || render)) {
    body.output.screenshot = { type: 'viewport' };
  }
...
async function doScrape(u, retryRender = false) {
  ...
  log.info(`Scraped OK ${u} — ${markdown.length} chars markdown, ${summary.length} chars summary`);
  ...
  return {
    ...
    credits: data.creditsUsed || res.totalCreditsUsed || '',
  };
}

// ====== Scrape with intelligent retry ======
async function doScrapeWithRetry(u) {
  ...
  if (isLikelyBlockedPage(result.markdown) && !render) {
    log.warning(`Content seems blocked for ${u}, retrying with JS rendering...`);
    try {
      const retryResult = await doScrape(u, true);
      if (!isLikelyBlockedPage(retryResult.markdown) && retryResult.markdown.length > 50) {
        return retryResult;
      }
    } catch (err) {
...
async function doSearch(q) {
  ...
  // FIXED: res.data is the array directly (not res.data.data)
  ...
  const limitFn = pLimit(MAX_CONCURRENCY);
  ...
  const tasks = basicResults.map((basic, i) =>
    limitFn(async () => {
      try {
        ...
        return {
          ...basic, markdown: '', summary: '', scrapeStatus: 'failed', scrapeError: err.message,
        };
      }
    })
  );
